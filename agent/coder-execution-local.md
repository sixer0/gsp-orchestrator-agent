...
### Canonical Skill Bullets

These skill bullets are canonical across all agent variants. Each maps the skill trigger phrase to the corresponding skill file:
...
- **[deploy-to-vercel](../../skills/deploy-to-vercel/SKILL.md)**: Deploy applications and websites to Vercel (preview or production). Use when the user requests deployment actions.
- **[diagnose](../../skills/diagnose/SKILL.md)**: Systematic diagnosis loop for hard bugs and performance regressions. Use when the user reports broken, failing, or slow behavior.
- **[finishing-a-development-branch](../../skills/finishing-a-development-branch/SKILL.md)**: Decide how to integrate finished work — merge, PR, or cleanup. Use after implementation is complete and tests pass.
- **[receiving-code-review](../../skills/receiving-code-review/SKILL.md)**: Process incoming code review feedback with technical rigor. Use when receiving review feedback, before implementing suggestions.
...
- **[setup-pre-commit](../../skills/setup-pre-commit/SKILL.md)**: Set up Husky pre-commit hooks with lint-staged, typechecking, and tests. Use when the user requests pre-commit hooks or commit-time checks.
- **[tdd](../../skills/tdd/SKILL.md)**: Test-driven development — red-green-refactor cycle with integration tests. Use when building features or fixing bugs test-first.
- **[using-git-worktrees](../../skills/using-git-worktrees/SKILL.md)**: Create isolated workspace via git worktree before implementation. Use when starting feature work that needs isolation.
...
- **[webapp-testing](../../skills/webapp-testing/SKILL.md)**: Playwright-based local web app testing with screenshots and browser logs. Use to verify frontend functionality or debug UI behavior.
