---
name: document-controller-local
...
| `document-analyst-local` | Assess relevance, quality, gaps | (always free) |
| `document-writer-local` | Create new documents | (always free) |
| `document-reader-local` | Extract from source documents | (always free) |
| `document-reviewer-local` | Integrate, revise, complete | (always free) |
| `document-converter-local` | Convert between formats | (always free) |
...
document-translator (parse & structure)
    ↓
[Optional: document-analyst-local] → assess relevance
    ↓
document-writer-local → create new document
    ↓
[Optional: document-reader-local] → extract from sources
    ↓
document-reviewer-local → integrate, revise, complete
    ↓
Final Response
```
...
2. document-writer-local
3. document-reviewer-local

### Simple Document Task (reading/extraction):
1. document-translator
2. document-reader-local
3. document-reviewer-local

### Full Document Task (with analysis):
1. document-translator
2. document-analyst-local (relevance check)
3. document-writer-local
4. document-reader-local
5. document-reviewer-local

### Conversion Task:
1. document-translator
2. document-converter-local

## How to Delegate

```
Task(subagent_type="document-xxx-local", prompt="
...
When `document-reviewer-local`, `document-reviewer`, `verifier-local`, `verifier`, `security-review-local`, `security-review`, or `test-expert-local` reports findings, use the following protocol:
...
- **Fix now** → re-delegate to `document-writer-local` / `document-analyst-local` with remediation tasks
...
```
document-reviewer-local returns: REVIEW_NEEDS_REWORK
    ↓
document-controller-local parses corrective feedback
    ↓
Delegates to relevant agents:
    - document-analyst-local: Address content/assumption issues
    - document-writer-local: Address writing/style issues
    ↓
Re-delegates to document-reviewer-local for re-check
    ↓
