...
### STEP 4.5: EXTERNAL RESEARCH SOURCE MAPPING

For tasks requiring external research, extend exploration beyond the local
project to map external sources. Use `websearch`, `webfetch`, and browser
automation (`agent-browser` skill) to discover and verify external resources.

#### External Source Categories

- **Third-party repositories**: Public GitHub/GitLab repos relevant to the
  technology domain — check stars, recent activity, license compatibility
- **Service registries**: npm, PyPI, crates.io, Maven — identify available
  packages, libraries, and their health metrics (downloads, issues, last update)
- **Ecosystem tools**: CLI tools, dev tools, monitoring, testing frameworks,
  CI/CD integrations in the technology ecosystem
- **External data sources**: Public APIs, open datasets, documentation portals,
  standards bodies relevant to the project domain

#### Output

Document findings in `research/01_explore.md` under an `## External Technology
Landscape` section. Map discovered sources to the canonical external research
dimensions:

| Discovery | Maps To |
|-----|--|-----|-----|-----|-----|-----|-----|-----|
| New frameworks, trending repos, emerging tools | **Technology Trends** (Dim 1) |
| Third-party services, API directories, SaaS platforms | **Vendor Evaluation** (Dim 3) |
...
### Technology Trends (Dim 1) — Source Mapping
| Source | URL | Description | Relevance |
|-----|-----|-------------|---|-----|-----|
| [name] | [url] | [what was found] | [how it matters to the task] |

### Vendor Evaluation (Dim 3) — Source Mapping
| Source | URL | Description | Relevance |
|-----|-----|-------------|---|-----|-----|
| [name] | [url] | [what was found] | [how it matters to the task] |
```

If no external sources are found, state:
"No external technology landscape sources identified for this task scope."
