...
The pm-analyst is the primary producer for business-domain external research.
Read the task's `research/03_external_vocabulary.md` for canonical terminology
and `research/02_collection.md` → `## External Research Findings` for gathered
data.

For each dimension in scope, produce business-domain analysis:

1. **Technology Trends** (Dim 1): Business implications of emerging technology —
   how new tech affects project timeline, budget, competitive advantage, team
   skill requirements, and maintenance cost projections.

2. **Technology Evaluation** (Dim 2): Technology options evaluated from a
   business perspective — TCO analysis, vendor lock-in risk, build-vs-buy ROI,
   team skill availability, long-term maintenance cost.

3. **Vendor Evaluation** (Dim 3): Business case for third-party vendors —
   contract terms analysis, SLA evaluation, pricing model comparison, vendor
   financial stability, integration cost estimation, data portability risk.

4. **Market Share & Positioning** (Dim 4): TAM/SAM/SOM analysis, market
   segmentation, growth projections (CAGR), revenue opportunity sizing,
   go-to-market positioning strategy, key player share distribution.

5. **Regulatory & Compliance** (Dim 5): Regulatory applicability assessment,
   compliance roadmap, certification requirements (SOC2, ISO 27001, HIPAA,
   PCI-DSS), audit readiness planning, jurisdiction-specific risk assessment.

6. **Competitive Landscape** (Dim 6): Competitor SWOT analysis, feature gap
   analysis, pricing comparison, differentiation strategy, market entry
   barriers, substitute product threats.

7. **Industry Trends** (Dim 7): Porter's Five Forces analysis, PESTLE assessment,
   analyst forecast synthesis (Gartner, Forrester, McKinsey), strategic
   recommendations. **Distinct from Dim 1**: Dim 1 covers the technology stack;
   Dim 7 covers the business domain.

Include findings in `research/03_analysis.md` under relevant sections. For
dimensions scoped to the masterplan phase, ensure findings are structured
for consumption by `pm-planner` and `data-analyst`.

### STEP 6: ASSESS TASK COMPLEXITY
Determine whether the task is simple or complex, with rationale.

### STEP 7: WRITE `research/03_analysis.md`
...
### STEP 8: CREATE `masterplan/02_plan.md` ONLY FOR COMPLEX TASKS
