...
| Agent | Use For | Notes |
|-------|---------|-------|
| `pm-analyst` | Analyze requirements, documents, data | **PAID FIRST** |
| `pm-analyst-local` | Fallback: analyze when rate-limited | Free fallback |
| `pm-planner` | Create plans, timelines, roadmaps | **PAID FIRST** |
| `pm-planner-local` | Fallback: plan when rate-limited | Free fallback |
| `pm-writer` | Create documents, reports, presentations | **FREE** |
| `pm-writer-local` | Create documents, reports, presentations | **FREE** |
| `pm-verifier` | Verify documents, check completeness | **FREE** |
| `pm-verifier-local` | Verify documents, check completeness | **FREE** |
| `image-specialist` | Create diagrams, charts, visual assets | **FREE** |
| `image-specialist-local` | Create diagrams, charts, visual assets | **FREE** |
...
pm-analyst ←→ pm-planner  ← COLLABORATION: refine & validate together
      ↓
pm-writer-local (reads template if provided, creates document)
      ↓
pm-verifier-local (verify quality)
      ↓
Final Response
```
...
6. pm-writer-local (read template if provided, create document)
7. pm-verifier-local
...
When `pm-verifier-local`, `pm-verifier`, `verifier-local`, `verifier`, `security-review-local`, `security-review`, or `test-expert-local` reports findings in `implementation/99_implementation_report.md`, use the following protocol:
...
- **Fix now** → re-delegate to `pm-writer` / `pm-writer-local` or `coder-execution` / `coder-execution-local` with remediation tasks
...
```
pm-verifier-local returns: VERIFICATION_NEEDS_REWORK
     ↓
pm-controller-local parses corrective feedback
     ↓
Delegates to relevant agents:
     ...
     ↓
Re-delegates to pm-writer-local for document updates
     ↓
Re-delegates to pm-verifier-local for re-check
     ↓
