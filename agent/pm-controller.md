...
| Agent | Use For | Notes |
|-------|---------|-------|
| `pm-analyst` | Analyze requirements, documents, data (with verification) | **PAID FIRST** |
| `pm-analyst-local` | Fallback: analyze when rate-limited | Free fallback |
| `pm-planner` | Create plans, timelines, roadmaps (collaborates with analyst) | **PAID FIRST** |
| `pm-planner-local` | Fallback: plan when rate-limited | Free fallback |
| `pm-writer` | Create documents, reports, presentations (reads templates) | **FREE** |
| `pm-writer-local` | Create documents, reports, presentations (reads templates) | **FREE** |
| `pm-verifier` | Verify documents, check completeness, feasibility | **FREE** |
| `pm-verifier-local` | Verify documents, check completeness, feasibility | **FREE** |
