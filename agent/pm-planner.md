...
for external research dimensions documented by `data-analyst` and `pm-analyst`.

#### Dimensions Relevant to Planning

1. **Technology Evaluation** (Dim 2): Technology choices that must be reflected
   in the implementation plan — specific versions, setup steps, configuration
   requirements, environment prerequisites.

2. **Vendor Evaluation** (Dim 3): Third-party integrations that need setup
   steps — API key provisioning, webhook configuration, vendor onboarding
   tasks, sandbox/testing environment setup, contract prerequisites.

3. **Regulatory & Compliance** (Dim 5): Compliance implementation steps —
   audit logging, encryption (at-rest and in-transit), data retention policies,
   access controls (RBAC/ABAC), right-to-deletion APIs, certification
   prerequisites, penetration testing gates.

#### Plan Output Requirements

Ensure `masterplan/02_plan.md` includes:

- `## External Dependencies` — all vendor/third-party integration steps with:
  - Vendor name, API/service, setup steps, prerequisites, verification
- `## Compliance Implementation` — regulatory requirement steps with:
  - Regulation name, required controls, implementation steps, audit gates
- Cross-references to canonical dimension numbers (Dim 2, Dim 3, Dim 5)
  for traceability back to analysis findings

If no external research dimensions affect the plan, state in the plan:
"No external dependencies or compliance requirements identified for this
implementation."
